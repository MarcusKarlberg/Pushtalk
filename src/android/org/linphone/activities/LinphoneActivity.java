...
import org.linphone.fragments.AccessSettingsDialogFragment;
...
public class LinphoneActivity extends LinphoneGenericActivity implements OnClickListener, ContactPicked, ActivityCompat.OnRequestPermissionsResultCallback, AccessSettingsDialogFragment.AccessSettingsDialogListener {
	...
	private void validateSettingsPassword(){
		LinphonePreferences prefs = getPrefs();
		String password = prefs.getSettingsPassword();
		boolean passwordEnabled = prefs.isSettingsPasswordEnabled();

		if(!prefs.isSettingsPasswordSet() || !passwordEnabled){
			fragment = new SettingsFragment();
		} else {
			if(settingsPassword.equals(password)){
				fragment = new SettingsFragment();
			} else {
				//Repeats if wrong password
				showAccessSettingsDialog();
			}
		}
	}

	private LinphonePreferences getPrefs(){
		return LinphonePreferences.instance();
	}

	private void showAccessSettingsDialog() {
		AccessSettingsDialogFragment dialogFragment = new AccessSettingsDialogFragment();
	...
	public void setPassword(String password) {
		android.util.Log.d(TAG, "setPassword: password typed = " + password);
