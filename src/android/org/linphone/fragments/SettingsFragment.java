...
public class SettingsFragment extends PreferencesListFragment {
	...
	private void initMediaEncryptionPreference(ListPreference pref) {
		...
		disableOrEnablePreference(findPreference(getString(R.string.pref_media_encryption_key)), mPrefs.isMediaEncryptionRemoteOverwrite());
	}
	...
	private void disablePreference(Preference preference){
		...
		preference.setIcon(R.drawable.padlock);
	}

	private void disableOrEnablePreference(Preference preference, boolean value){
		if(value){
			preference.setEnabled(false);
			preference.setIcon(R.drawable.padlock);
		}
	...
	private void initAudioSettings() {
		...
		Core lc = LinphoneManager.getLcIfManagerNotDestroyedOrNull();
		int audioCodecSectionNumber = 0;
		String audioCodecSection = "audio_codec_";
		for (final PayloadType pt : lc.getAudioPayloadTypes()) {
			...
			codec.setSummary(pt.getClockRate() + " Hz " + " (" + pt.getChannels() + "ch)");
			...
			if(mPrefs.isAudioCodecRemoteOverwrite(audioCodecSection + audioCodecSectionNumber)){
				disablePreference(codecs.getPreference(audioCodecSectionNumber));
			}

			audioCodecSectionNumber++;
		}
		...
		EditTextPreference micGain = (EditTextPreference) findPreference(getString(R.string.pref_mic_gain_key));
		String micGainSummary = Float.toString(lc.getMicGainDb());
		micGain.setSummary(micGainSummary);
		micGain.setText(micGainSummary);

		EditTextPreference playbackGain = (EditTextPreference) findPreference(getString(R.string.pref_playback_gain_key));
		String playBackGainSummary = Float.toString(lc.getPlaybackGainDb());
		playbackGain.setSummary(playBackGainSummary);
		playbackGain.setText(playBackGainSummary);
		...
		if(mPrefs.getRemoteProvisioningUrl() != null){
			disableOrEnablePreference(bitrateLimit, mPrefs.isCodecBitrateLimitRemoteOverwrite());
			disableOrEnablePreference(micGain, mPrefs.isMicGainRemoteOverwrite());
			disableOrEnablePreference(playbackGain, mPrefs.isPlaybackGainRemoteOverwrite());
			disableOrEnablePreference(adaptiveRateControl, mPrefs.isPlaybackGainRemoteOverwrite());

				if(mPrefs.isEchoCancellationRemoteOverwrite()){
					disablePreference(echoCancellation);
					disablePreference(echoCalibration);
					disablePreference(findPreference(getString(R.string.pref_echo_tester_key)));
				}
		}
	}
	...
	private void setAudioPreferencesListener() {
		...
		findPreference(getString(R.string.pref_mic_gain_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			@Override
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				String micGainValue = newValue.toString();
				Core lc = LinphoneManager.getLcIfManagerNotDestroyedOrNull();
				if(lc != null){
					lc.setMicGainDb(Float.parseFloat(micGainValue));

					if(preference.getKey().equals("pref_mic_gain_key")){
						//String summaryValue = preference.getContext().getResources().getString(R.string.pref_mic_gain_summary);
						preference.setSummary(micGainValue);
					}
				}
				return true;
			}
		...
		findPreference(getString(R.string.pref_playback_gain_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			@Override
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				String playbackGainValue = newValue.toString();
				Core lc = LinphoneManager.getLcIfManagerNotDestroyedOrNull();

				if(lc != null){
					lc.setPlaybackGainDb(Float.parseFloat(playbackGainValue));

					if(preference.getKey().equals("pref_playback_gain_key")){
						preference.setSummary(playbackGainValue);
					}
				}
	...
	private void initVideoSettings() {
		...
		int videoCodecSectionNumber = 0;
		String audioCodecSection = "video_codec_";
		...
		for (final PayloadType pt : lc.getVideoPayloadTypes()) {
			...
			if(mPrefs.isVideoCodecRemoteOverwrite(audioCodecSection + videoCodecSectionNumber)){
				disablePreference(codecs.getPreference(videoCodecSectionNumber));
			}

			videoCodecSectionNumber++;
		}
		...
		if (mCodecDownloader.isOpenH264DownloadEnabled() && !h264IsHere) {
			...
			codec.setTitle("OpenH264");
			codec.setSummary(mCodecDownloader.getLicenseMessage());
			//To disable H264 from remote provisioning
			disableOrEnablePreference(codec, mPrefs.isVideoSectionRemoteOverwrite());
		...
		disableOrEnablePreference(findPreference(getString(R.string.pref_video_enable_key)), mPrefs.isEnableVideoRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_video_initiate_call_with_video_key)), mPrefs.isInitiateVideoCallsRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_video_automatically_accept_video_key)), mPrefs.isAcceptIncomingVideoRemoteOverwrite());
		disableOrEnablePreference(videoPresetPref, mPrefs.isVideoPresetRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_preferred_video_fps_key)), mPrefs.isPreferredFpsRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_preferred_video_size_key)), mPrefs.isPreferredVideoSizeRemoteOverwrite());
		disableOrEnablePreference(bandwidth, mPrefs.isBandwidthLimitRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_overlay_key)), mPrefs.isVideoOverlayRemoteOverwrite());
	}
	...
	private void initCallSettings() {
		...
		CheckBoxPreference autoSpeaker = (CheckBoxPreference) findPreference(getString(R.string.pref_auto_speaker_key));
		...
		autoSpeaker.setChecked(mPrefs.isAutoSpeakerEnabled());

		EditTextPreference doorIntercomNumber = (EditTextPreference) findPreference(getString(R.string.pref_door_intercom_key));
		doorIntercomNumber.setSummary(mPrefs.getDoorIntercomNumber());
		doorIntercomNumber.setText(mPrefs.getDoorIntercomNumber());

		EditTextPreference doorIntercomDtfTone = (EditTextPreference) findPreference(getString(R.string.pref_door_intercom_dtf_tone_key));
		doorIntercomDtfTone.setSummary(mPrefs.getDoorIntercomDtfTone());
		doorIntercomDtfTone.setText(mPrefs.getDoorIntercomDtfTone());

		disableOrEnablePreference(deviceRingtone, mPrefs.isDeviceRingtoneEnabledRemoteOverwrite());
		disableOrEnablePreference(sipInfo, mPrefs.isSendOutbandSipInfoDtmfRemoteOverwrite());
		disableOrEnablePreference(rfc2833, mPrefs.isSendInbandDtmfRFC2833RemoteOverwrite());
		disableOrEnablePreference(autoAnswer, mPrefs.isAutoAnswerIncomingCallsRemoteOverwrite());
		disableOrEnablePreference(autoAnswerTime, mPrefs.isAutoAnswerTimeRemoteOverwrite());
		disableOrEnablePreference(incTimeout, mPrefs.isIncomingCallTimeoutRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_voice_mail_key)), mPrefs.isVoiceMailURIRemoteOverwrite());
		disableOrEnablePreference(dialerCall, mPrefs.isUseLinphoneAsDefaultPhoneAppRemoteOverwrite());
		disableOrEnablePreference(doorIntercomNumber, mPrefs.isDoorIntercomNumberRemoteOverwrite());
		disableOrEnablePreference(doorIntercomDtfTone, mPrefs.isDoorIntercomDtfToneRemoteOverwrite());
		disableOrEnablePreference(autoSpeaker, mPrefs.isAutoSpeakerEnabledRemoteOverwrite());
	}
	...
	private void setCallPreferencesListener() {
		...
		findPreference(getString(R.string.pref_door_intercom_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			@Override
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				if(preference.getKey().equals(getString(R.string.pref_door_intercom_key))){
					String value = newValue.toString();
					mPrefs.setDoorIntercomNumber(value);
		...
		findPreference(getString(R.string.pref_door_intercom_dtf_tone_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			@Override
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				if(preference.getKey().equals(getString(R.string.pref_door_intercom_dtf_tone_key))){
					String value = newValue.toString();
					mPrefs.setDoorIntercomDtfTone(value);
		...
		findPreference(getString(R.string.pref_auto_speaker_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			@Override
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				boolean use = (Boolean) newValue;
				mPrefs.setAutoSpeakerEnabled(use);
				return true;
			}
		});
	...
	private void initChatSettings() {
		...
		disableOrEnablePreference(findPreference(getString(R.string.pref_use_lime_encryption_key)), mPrefs.isLimeEncryptionRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_image_sharing_server_key)), mPrefs.isSharingServerRemoteOverwrite());
	}
	...
	private void initNetworkSettings() {
		...
		disableOrEnablePreference(findPreference(getString(R.string.pref_wifi_only_key)), mPrefs.isWifiOnlyRemoteOverwrite());
		disableOrEnablePreference(stun, mPrefs.isStunTurnServerRemoteOverwrite("nat_policy_0"));
		disableOrEnablePreference(ice, mPrefs.isEnableIceRemoteOverwrite("nat_policy_0"));
		disableOrEnablePreference(turn, mPrefs.isEnableTurnRemoteOverwrite("nat_policy_0"));
		disableOrEnablePreference(turnUsername,mPrefs.isStunServerUsernameRemoteOverwrite("nat_policy_0"));
		disableOrEnablePreference(randomPort, mPrefs.isUseRandomPortsRemoteOverwrite());
		disableOrEnablePreference(sipPort, mPrefs.isSipPortRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_push_notification_key)), mPrefs.isPushNotificationRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_ipv6_key)), mPrefs.isAllowIPv6ORemoteOverwrite());
	}
	...
	private void initAdvancedSettings() {
		...
		((CheckBoxPreference)findPreference(getString(R.string.pref_settings_password_enabled_key))).setChecked(mPrefs.isSettingsPasswordEnabled());

		EditText editText = ((EditTextPreference) findPreference(getString(R.string.pref_settings_password_key))).getEditText();
		String maskedPassword = editText.getTransformationMethod().getTransformation(mPrefs.getSettingsPassword(), editText).toString();

		setPreferenceDefaultValueAndSummary(R.string.pref_settings_password_key, maskedPassword);
		setPreferenceDefaultValueAndSummary(R.string.pref_remote_provisioning_key, mPrefs.getRemoteProvisioningUrl());

		if(mPrefs.isDisplayNameRemoteOverwrite()){
			setPreferenceDefaultValueAndSummary(R.string.pref_display_name_key, mPrefs.getDisplayNameRemoteOverwrite());
			mPrefs.setDefaultDisplayName(mPrefs.getDisplayNameRemoteOverwrite());
		}else{
			setPreferenceDefaultValueAndSummary(R.string.pref_display_name_key, mPrefs.getDefaultDisplayName());
		}

		if(mPrefs.isUsernameRemoteOverwrite()){
			setPreferenceDefaultValueAndSummary(R.string.pref_user_name_key, mPrefs.getUsernameRemoteOverwrite());
			mPrefs.setDefaultUsername(mPrefs.getUsernameRemoteOverwrite());
		}else{
			setPreferenceDefaultValueAndSummary(R.string.pref_user_name_key, mPrefs.getDefaultUsername());
		}


		disableOrEnablePreference(findPreference(getString(R.string.pref_debug_key)), mPrefs.isDebugRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_java_debug_key)), mPrefs.isJavaLoggerRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_friendlist_subscribe_key)), mPrefs.isFriendsListSubscriptionOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_background_mode_key)), mPrefs.isBackgroundModeOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_service_notification_key)), mPrefs.isEnableServiceNotificationOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_autostart_key)), mPrefs.isAutoStartRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_remote_provisioning_key)), mPrefs.isRemoteProvisioningUriOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_user_name_key)), mPrefs.isUsernameRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_settings_password_enabled_key)), mPrefs.isSettingsPasswordEnabledRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_settings_password_key)), mPrefs.isSettingsPasswordRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_display_name_key)), mPrefs.isDisplayNameRemoteOverwrite());
		disableOrEnablePreference(findPreference(getString(R.string.pref_user_name_key)), mPrefs.isUsernameRemoteOverwrite());
	}

	private void setAdvancedPreferencesListener() {

		findPreference(getString(R.string.pref_settings_password_enabled_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			@Override
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				boolean value = (Boolean) newValue;
				int use = value ? 1 : 0;
				mPrefs.SetSettingsPasswordEnabled(use);
				return true;
			}
		});

		findPreference(getString(R.string.pref_settings_password_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			@Override
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				if(preference.getKey().equals(getString(R.string.pref_settings_password_key))){
					String password = newValue.toString();
					EditText editText = ((EditTextPreference) preference).getEditText();
					String maskedPassword = editText.getTransformationMethod().getTransformation(newValue.toString(), editText).toString();

					mPrefs.setSettingsPassword(password);
					preference.setSummary(maskedPassword);
				}
